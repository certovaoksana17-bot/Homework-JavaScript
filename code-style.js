...
let resultMultiply = Multiply(5, 10);
...
// функция должна возвращать сообщение "Форма успешно отправлена".
 ...
 function validateObject(obj, requiredFields) {
  for (const field of requiredFields) {
    if (!obj.hasOwnProperty(field) || obj[field] === '' || obj[field] === undefined || obj[field] === null) {
      throw new Error(`Поле "${field}" не заполнено!`);
    }
  }
  return true; 
...
try {
  validateObject(myObject, required);
